...
description: "Why xaker's code looks the way it does — the polymorphism registries, naming rules, numerical-stability choices, and what we deliberately did not build."
permalink: "/design_decisions/"
---

**Audience: contributors who wonder *why* a particular design choice was made.**
...
- XSA — paper in preparation; companion implementation in this
  repository ([`sachncs/xaker`](https://github.com/sachncs/xaker)).
  An arXiv identifier will replace this entry on the paper's
  submission.
...
queries and keys. Lives in `xaker/attention/standard.py`. The
baseline against which `Xsa`, `Fused`, and `Linear` are
benchmarked.
...
Lives in `xaker/attention/xsa.py`.
...
Lives in `xaker/attention/fused.py`.
...
## Next steps
- [Architecture](/xaker/architecture/) — the module layout the principles produced.
- [Mathematical foundations](/xaker/math/) — the math behind every choice here.
