...
from xaker.bench.bench import converge
...
class TestConverge:
    """``bench.converge`` actually runs PCG (issue #38)."""

    def test_converge_on_fused(self) -> None:
        """converge() returns finite iters_mean when called on a Fused block."""
        cfg = Config(dim=64, heads=4, precond="diagonal")
        attn = BLOCK["fused"](cfg).eval()
        x = torch.randn(1, 16, cfg.dim)
        conv, mean, std = converge(attn, x, ctx=Ctx(device="cpu"), runs=2, iters=10, tol=1e-3)
        assert mean > 0.0
        assert std >= 0.0

    def test_converge_reports_non_fused_as_not_converged(self) -> None:
        """Non-Fused callers return (False, 0.0, 0.0)."""
        cfg = Config(dim=64, heads=4)
        attn = BLOCK["standard"](cfg).eval()
        x = torch.randn(1, 16, cfg.dim)
        conv, mean, std = converge(attn, x, ctx=Ctx(device="cpu"), runs=1)
