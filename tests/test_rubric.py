...
from xaker.rubric.grader import novelty
...
def test_novelty_evidence_mentions_fused_not_laker() -> None:
    """The novelty grader's evidence references ``fused``, not the legacy ``laker``."""
    from pathlib import Path
    dim = novelty(Path("."))
    assert "fused" in dim.score.evidence
    assert "laker" not in dim.score.evidence


def test_novelty_caps_at_three() -> None:
    """The novelty score never exceeds 3 even when both novel files exist."""
    from pathlib import Path
    dim = novelty(Path("."))
    assert dim.score.value <= 3