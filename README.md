...
- Live test and coverage counts are tracked in [`STATUS.md`](STATUS.md);
  the CI gate is `--cov-fail-under=90`. Paper-worthiness rubric:
  `xaker-validate` (17 / 18 by default).
...
Current test count, coverage, and rubric grade are tracked in
[`STATUS.md`](STATUS.md); CI keeps `STATUS.md` in sync with the
test results on every push to `master`.
...
@article{xsa_paper,
  ...
  author = {Sachin},
  journal = {GitHub repository, \url{https://github.com/sachncs/xaker}},
  year = {2026},
  note = {Paper in preparation; arXiv identifier will replace this entry on submission.}
}
```
