...
class Fused(Base):
    ...
    def attend(
        ...
        # Bump Fast preconditioner step counter (per-Fast instance;
        # every Fused block constructs its own ``Make(config)`` so the
        # counter is local to this block, never shared across layers).
        fast = self.precon
