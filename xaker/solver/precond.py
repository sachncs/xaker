...
class Fast(nn.Module):
    ...
    The cached payload is invalidated by *either* of two signals:

    * the step counter modulo ``Config.freq`` is not zero, and
    * at least one of ``lr_base``, ``lr_imp``, or ``scale`` has
      changed since the last build (detected by ``Tensor.data_ptr()``
      identity).

    This means mutations to the learnable factors between two
    ``build`` calls always rebuild the preconditioner, even when
    ``freq > 1`` and the step counter would otherwise skip the work.
    """
    ...
    def __init__(self, config) -> None:
        ...
        # ``self.iter`` is registered as a buffer above; this annotation
        # only documents its type for static checkers (single source of
        # truth to avoid the dual declaration that confused issue #45).
        self.cache: Cache | None = None
        self._cache_keys: tuple[int, ...] | None = None

    def _signature(self) -> tuple[int, ...]:
        """Tensor identity tuple used to invalidate the cache.

        Returns a tuple of ``Tensor.data_ptr()`` values for the
        learnable factors plus ``scale``. Mutation that reuses the
        same storage (in-place) will keep the pointers stable, but
        optimizer steps typically allocate new storage, so this
        signal is sufficient to detect trained updates.
        """
        return (
            self.lr_base.data_ptr(),
            self.lr_imp.data_ptr(),
            self.scale.data_ptr(),
        )

    def build(self, kernel: torch.Tensor, lam: torch.Tensor, length: int) -> Cache:
        sig = self._signature()
        step = int(self.iter[0])
        cached_step_ok = self.freq > 0 and step % self.freq != 0
        sig_ok = self._cache_keys == sig
        if self.cache is not None and cached_step_ok and sig_ok:
            return self.cache
        diag = torch.diagonal(kernel, dim1=-2, dim2=-1)
    ...
    def reset_cache(self) -> None:
        """Invalidate the cached payload and reset the step counter.

        Call this method when an external event invalidates the
        preconditioner's view of the world, for example after an
        optimizer step when ``Config.freq > 1`` so the next
        ``build`` will recompute the payload.
        """
        self.cache = None
